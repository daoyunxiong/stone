...
// Do replace this function with something useful. You can
// also move this function to a diffrent file in your project
// and include the header file here.
int GPS_transmit(const char *message) {
	size_t pos = 0;
	while(message[pos] != '\0'  &&  pos < 1000) {
		pos++;
	}
...
static void merge_ddmmyy(char *ddmmyy, struct gps_data_t *gpsdata)
/* sentence supplied ddmmyy, but no century part */
{
    if (gpsdata->nmea.date.tm_year == 0)
	gpsdata->nmea.date.tm_year = (CENTURY_BASE + DD(ddmmyy+4)) - 1900;
    gpsdata->nmea.date.tm_mon = DD(ddmmyy+2)-1;
    gpsdata->nmea.date.tm_mday = DD(ddmmyy);
}

static void merge_hhmmss(char *hhmmss, struct gps_data_t *gpsdata)
/* update from a UTC time */
{
    int old_hour = gpsdata->nmea.date.tm_hour;

    gpsdata->nmea.date.tm_hour = DD(hhmmss);
	if (gpsdata->nmea.date.tm_hour < old_hour)	/* midnight wrap */ 
	gpsdata->nmea.date.tm_mday++;
    gpsdata->nmea.date.tm_min = DD(hhmmss+2);
    gpsdata->nmea.date.tm_sec = DD(hhmmss+4);
    gpsdata->nmea.subseconds = atof(hhmmss+4) - gpsdata->nmea.date.tm_sec;
}
...
static gps_mask_t processGPRMC(int count, char *field[], struct gps_data_t *gpsdata)
/* Recommend Minimum Course Specific GPS/TRANSIT Data */
{
	...
	if (count > 9) {
	    merge_ddmmyy(field[9], gpsdata);
	    merge_hhmmss(field[1], gpsdata);
	    mask |= TIME_SET;
	    gpsdata->fix.time = (double)mkgmtime(&gpsdata->nmea.date)+gpsdata->nmea.subseconds;
	    if (gpsdata->sentence_time != gpsdata->fix.time)
		mask |= CYCLE_START_SET;
	    gpsdata->sentence_time = gpsdata->fix.time;
	}
	do_lat_lon(&field[3], gpsdata);
	mask |= LATLON_SET;
	gpsdata->fix.speed = atof(field[7]) * KNOTS_TO_MPS;
	gpsdata->fix.track = atof(field[8]);
	 ...
	 */
	if (gpsdata->status == STATUS_NO_FIX) {
	    gpsdata->status = STATUS_FIX;	/* could be DGPS_FIX, we can't tell */
	    mask |= STATUS_SET;
	}
	if (gpsdata->fix.mode < MODE_2D) {
...
static gps_mask_t processGPGLL(int count, char *field[], struct gps_data_t *gpsdata)
/* Geographic position - Latitude, Longitude */
{
	...
	int newstatus = gpsdata->status;

	mask = 0;
	merge_hhmmss(field[5], gpsdata);
	if (gpsdata->nmea.date.tm_year == 0) 
...
static gps_mask_t processGPGGA(int c UNUSED, char *field[], struct gps_data_t *gpsdata)
/* Global Positioning System Fix Data */
{
    ...
    */
    ...
    gpsdata->status = atoi(field[6]);
    gpsd_report(LOG_PROG, "GPGGA sets status %d\n", gpsdata->status);
    mask = STATUS_SET;
    if (gpsdata->status > STATUS_NO_FIX) {
	char *altitude;
	double oldfixtime = gpsdata->fix.time;
	...
	do_lat_lon(&field[2], gpsdata);
	mask |= LATLON_SET;
        gpsdata->satellites_used = atoi(field[7]);
	altitude = field[9];
	/*
	...
	if (altitude[0] == '\0') {
		...
		gpsdata->fix.mode = gpsdata->status ? MODE_2D : MODE_NO_FIX; 
		mask |= MODE_SET;
	    }
	} else {
	    double oldaltitude = gpsdata->fix.altitude;

	    gpsdata->fix.altitude = atof(altitude);
	    mask |= ALTITUDE_SET;
	    /*
	     * This is a bit dodgy.  Technically we shouldn't set the mode 
	     * bit until we see GSA.  But it may be later in the cycle,
	     ...
	     * going on.
	     */
	    if (isnan(oldaltitude)!=0 || gpsdata->fix.time==oldfixtime)
		gpsdata->fix.climb = 0;
	    else {
		gpsdata->fix.climb = (gpsdata->fix.altitude-oldaltitude)/(gpsdata->fix.time-oldfixtime);
	    }
	    mask |= CLIMB_SET;
	}
	if (strlen(field[11]) > 0) {
	   gpsdata->separation = atof(field[11]);
	} else {
...
static gps_mask_t processGPGSA(int count, char *field[], struct gps_data_t *gpsdata)
/* GPS DOP and Active Satellites */
{
     ...
     */
    if (gpsdata->fix.mode == 0 && field[2][0] == 'E')
        mask = 0;
    else
        mask = MODE_SET;
    gpsd_report(LOG_PROG, "GPGSA sets mode %d\n", gpsdata->fix.mode);
    gpsdata->pdop = atof(field[count-3]);
    gpsdata->hdop = atof(field[count-2]);
    gpsdata->vdop = atof(field[count-1]);
    gpsdata->satellites_used = 0;
    memset(gpsdata->used,0,sizeof(gpsdata->used));
    /* the magic 6 here counts the tag, two mode fields, and the DOP fields */
    for (i = 0; i < count - 6; i++) {
        int prn = atoi(field[i+3]);
        if (prn > 0)
	    gpsdata->used[gpsdata->satellites_used++] = prn;
    }
...
static gps_mask_t processGPGSV(int count, char *field[], struct gps_data_t *gpsdata)
/* GPS Satellites in View */
{
    ...
    gpsdata->nmea.await = atoi(field[1]);
    if (sscanf(field[2], "%d", &gpsdata->nmea.part) < 1) {
		gpsd_zero_satellites(gpsdata);
        return ERROR_SET;
    } else if (gpsdata->nmea.part == 1)
		gpsd_zero_satellites(gpsdata);

    for (fldnum = 4; fldnum < count; ) {
	if (gpsdata->satellites >= MAXCHANNELS) {
	    gpsd_report(LOG_ERROR, "internal error - too many satellites!\n");
	    //sk gpsd_zero_satellites(&session->gpsdata);
	    break;
	}
	gpsdata->PRN[gpsdata->satellites]       = atoi(field[fldnum++]);
	gpsdata->elevation[gpsdata->satellites] = atoi(field[fldnum++]);
	gpsdata->azimuth[gpsdata->satellites]   = atoi(field[fldnum++]);
	gpsdata->ss[gpsdata->satellites]        = atoi(field[fldnum++]);
	/*
	 ...
	 * a multiple of 4.
	 */
	if (gpsdata->PRN[gpsdata->satellites] != 0)
	    gpsdata->satellites++;
    }
    if (gpsdata->nmea.part == gpsdata->nmea.await && atoi(field[3]) != gpsdata->satellites)
	gpsd_report(LOG_WARN, "GPGSV field 3 value of %d != actual count %d\n",
		    atoi(field[3]), gpsdata->satellites);

    /* not valid data until we've seen a complete set of parts */
    if (gpsdata->nmea.part < gpsdata->nmea.await) {
	gpsd_report(LOG_PROG, "Partial satellite data (%d of %d).\n", gpsdata->nmea.part, gpsdata->nmea.await);
	return ERROR_SET;
    }
    /*
     ...
     */
    for (n = 0; n < gpsdata->satellites; n++)
	if (gpsdata->azimuth[n] != 0)
	    goto sane;
...
static gps_mask_t processPGRME(int c UNUSED, char *field[], struct gps_data_t *gpsdata)
...
// sk
    ...
    gpsdata->fix.eph = atof(field[1]) * (GPSD_CONFIDENCE/CEP50_SIGMA);
    gpsdata->fix.epv = atof(field[3]) * (GPSD_CONFIDENCE/CEP50_SIGMA);
    gpsdata->epe = atof(field[5]) * (GPSD_CONFIDENCE/CEP50_SIGMA);
...
static gps_mask_t processGPZDA(int c UNUSED, char *field[], struct gps_data_t *gpsdata)
/* Time & Date */
{
     ...
     */
    merge_hhmmss(field[1], gpsdata);
    gpsdata->nmea.date.tm_year = atoi(field[4]) - 1900;
    gpsdata->nmea.date.tm_mon = atoi(field[3])-1;
    gpsdata->nmea.date.tm_mday = atoi(field[2]);
    gpsdata->fix.time = (double)mkgmtime(&gpsdata->nmea.date)+gpsdata->nmea.subseconds;
    if (gpsdata->sentence_time != gpsdata->fix.time)
	mask |= CYCLE_START_SET;
    gpsdata->sentence_time = gpsdata->fix.time;
...
#ifdef TNT_ENABLE
static gps_mask_t processTNTHTM(int c UNUSED, char *field[], struct gps_data_t *gpsdata)
{
    /*
	...
	*hh          mandatory nmea_checksum
    ...
    gpsdata->fix.track = atof(field[1]);
    gpsdata->headingStatus = *field[2];
    gpsdata->fix.climb = atof(field[3]);
    gpsdata->pitchStatus = *field[4];
    gpsdata->fix.speed = atof(field[5]);
    gpsdata->rollStatus = *field[6];
    gpsdata->fix.altitude = atof(field[7]);
    gpsdata->horzField = atof(field[8]);
    gpsdata->fix.mode = MODE_3D;
    mask |= (STATUS_SET | MODE_SET | TRACK_SET | SPEED_SET | CLIMB_SET | ALTITUDE_SET);
    gpsdata->status = STATUS_FIX;	/* could be DGPS_FIX */

    gpsd_report(LOG_RAW, "Heading %lf  %c.\n", gpsdata->fix.track, gpsdata->headingStatus);
    return mask;
}
#endif /* TNT_ENABLE */
...
gps_mask_t nmea_parse(char *sentence, struct gps_data_t *gpsdata)
/* parse an NMEA sentence, unpack it into a data structure */
{
    typedef gps_mask_t (*nmea_decoder)(int count, char *f[], struct gps_data_t *gpsdata);
    static struct {
		char *name;
    ...
    for (i = 0; i < (unsigned)(sizeof(nmea_phrase)/sizeof(nmea_phrase[0])); ++i) {
        ...
        if (strcmp(nmea_phrase[i].name, s) == 0) {
		    if (nmea_phrase[i].decoder!=NULL && (count >= nmea_phrase[i].nf)) {
				retval = (nmea_phrase[i].decoder)(count, field, gpsdata);
				strncpy(gpsdata->tag, nmea_phrase[i].name, MAXTAGLEN);
				gpsdata->sentence_length = strlen(sentence);
		    } else
