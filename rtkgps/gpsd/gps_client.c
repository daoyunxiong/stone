    ...
    if (gps_waiting(&gps_data, 100000)) {
      ...
      if ((rc = gps_read(&gps_data)) == -1) {
        ...
        if ((gps_data.status == STATUS_FIX) &&
          ...
          // timestamp!
          printf("status:%d, latitude: %4.10f, longitude: %4.10f, speed: %f, "
                 "timestamp: %f\n",
                 gps_data.status, gps_data.fix.latitude, gps_data.fix.longitude,
                 gps_data.fix.speed,
                 gps_data.fix
                     .time); // EDIT: Replaced tv.tv_sec with gps_data.fix.time
        } else {
