...
*
*/
double UTMCentralMeridian(int zone) { return DegToRad(-183.0 + (zone * 6.0)); }
...
void LatLonToUTMXY(double lat, double lon, int zone, UTMCoor *xy) {
  g_zone = floor((lon + 180.0) / 6) + 1;
  //	cout << "g_zone" << g_zone << endl;
  lat = DegToRad(lat);
  lon = DegToRad(lon);
  MapLatLonToXY(lat, lon, UTMCentralMeridian(g_zone), xy);
...
*
*/
void UTMXYToLatLon(double x, double y, int zone, int southhemi,
                   WGS84Corr *latlon) {
  double cmeridian;
