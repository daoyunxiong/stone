...
fprintf(fp, "%d,%4.10lf,%4.10lf,%d,%d,%f,%f,%f,%d,%d\n",rtk_data.pos_llh.tow, rtk_data.pos_llh.lat, rtk_data.pos_llh.lon, rtk_data.pos_llh.flags, rtk_data.pos_llh.n_sats, rtk_data.heading_kalman, rtk_data.heading_mag, rtk_data.heading_data, rtk_data.vel_ned_data.n, rtk_data.vel_ned_data.e);
fflush(fp);
...
void IMU_Update(rtk_data_t &rtk_data) {
    ...
    Point2f imu_point;
    imu_point.x = rtk_data.utmcoor.x -OFFSET_X + 500;
    imu_point.y = rtk_data.utmcoor.y -OFFSET_Y;
    imu_point.y = -imu_point.y + 500;

//    printf("%f %f\n", imu_point.x, imu_point.y);
    circle(image_imu_g, imu_point, 1, Scalar(255, 0, 0));
    imshow("imu_complement ", image_imu_g);
