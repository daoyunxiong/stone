...
double RTK_GPS::yaw;
double RTK_GPS::roll;
double RTK_GPS::pitch;
double RTK_GPS::xcoor;
double RTK_GPS::ycoor;
double RTK_GPS::vn_g;
double RTK_GPS::ve_g;
        ...
        case SBP_MSG_VEL_NED: //output ned coordinate heading angle.
           ...
           // printf("gps velocity heading: %f \n", data->heading_data);
            ...
            if(data.vel_ned_data.flags != 0){
              vn_g = (float)data.vel_ned_data.n; 
              ve_g = (float)data.vel_ned_data.e;
            }
            break;
...
*/
    ...
    double sumx_acc, sumy_acc, sumz_acc; 
    for(int j = 0; j < 8; j++){
...
//    if(abs(gy_i) < 10) gy_i = 0;
//    if(abs(gz_i) < 10) gz_i = 0;
    ax = RTK_GPS::convertRawAcceleration(ax_i) * 9.8;
    ay = RTK_GPS::convertRawAcceleration(ay_i) * 9.8;
    az = RTK_GPS::convertRawAcceleration(az_i) * 9.8;
...
//update roll pitch
    ...
    double gyroXrate = gx; // Convert to deg/s
    double gyroYrate = gy; // Convert to deg/s
    double gyroZrate = gz; // Convert to deg/s
...
//float  heading_ahrs = filter.getYaw();
    ...
    newangle = (float) data.heading_mag;
...
//  if(angle >360)
//    angle = angle - 360;
    data.heading_kalman = angle;
    std::cout  <<  angle << ", " << data.heading_kalman << std::endl;
    
 //   printf("%f, %f \n\n", angle , rtk_data.heading_kalman);
#ifndef DEBUG        
...
#ifdef DEBUG
  //  printf("tow acc gyo mag %d,%f,%f,%f,%d,%d,%d,%d,%d,%d \n", data.imu_data_raw.tow, ax, ay, az, gx_i, gy_i, gz_i, mx_i, my_i, mz_i);
#endif

//  double accn = ax * cos((90 - angle) * M_PI/180) + ay * sin((90 - angle) * M_PI/180);
//  double acce = ay * cos((90 - angle) * M_PI/180) - ax * sin((90 - angle) * M_PI/180);

  double accn = ax * cos(M_PI/3) + ay * sin(M_PI/3);
  double acce = ay * cos(M_PI/3) - ax * sin(M_PI/180);

  printf("%f %f %f %f %f accn acce\n",angle, accn, acce, ax, ay);
  ycoor += vn_g * dt + 0.5 * accn * dt * dt;
  xcoor += vn_g * dt + 0.5 * acce * dt * dt;
